...
| BRK-001 | **Connect eviction commits before verification** — `connect` (`api/brokers.py:644`) calls `_evict_existing_brokers` (deletes the user's existing broker, `db.flush` uncommitted, `:248-271`) BEFORE `verify_connection` (`:662`). On a verify failure the `except` branch `db.commit()`s (`:674`) the pending eviction, while the new account is only `db.add`-ed on success (`:677`). Net: a failed replace leaves the user with **NO broker** (old committed-deleted, new never persisted, listener already stopped). Same pattern in `snaptrade_finish` (`:557`/`:585`). | TC-BRK-001-003 / 001-016 | **Confirmed by code review** — high severity. Runtime repro blocked by the no-outbound rule (verify needs a live Alpaca/IBKR/SnapTrade call); recommend the app team reorder evict-after-verify or wrap in a savepoint. |
