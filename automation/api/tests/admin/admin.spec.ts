...
test.describe('Admin operations', () => {
  ...
  test('TC-ADMIN-001-004 every /api/admin route is admin-only (403 for non-admin, 401 unauth) @admin @api @P0 @security', async ({ api, config }, info) => {
    meta(info, 'ADMIN-001');
  ...
  test('TC-ADMIN-001-001 user management — list/search, activate/deactivate, role, business-name @admin @api @P1 @integration', async ({ api, config }, info) => {
    meta(info, 'ADMIN-001');
    const admin = await makeAdmin(api, config);
  ...
  test('TC-ADMIN-001-002 admin safety guards — no deactivate-admin, no self-role-change, business-name trader-only @admin @api @P1 @negative', async ({ api, config }, info) => {
    meta(info, 'ADMIN-001');
    const admin = await makeAdmin(api, config);
    ...
    try {
      ...
      expect((await a.changeBusinessName(api, admin.token, sub.id, 'x')).status(), 'business-name trader-only').toBe(400);
    } finally {
      deleteUser(config, admin.email);
  ...
  test('TC-ADMIN-001-003 admin user mutations are NOT written to the audit log (documented gap) @admin @api @P2 @observability', async ({ api, config }, info) => {
    meta(info, 'ADMIN-001');
    const admin = await makeAdmin(api, config);
    const targetU = makeUser('trader');
    const target = await auth.registerAndLogin(api, targetU);
  ...
  test('TC-ADMIN-002-002 dashboards + health — stats, user-counts, daily-pnl, broker/listener health @admin @api @P2 @integration', async ({ api, config }, info) => {
    meta(info, 'ADMIN-002');
    const admin = await makeAdmin(api, config);
    try {
      expect((await a.stats(api, admin.token)).status()).toBe(200);
  ...
  test('TC-ADMIN-003-001 rejected-order triage lists rejections and never exposes credentials @admin @api @P2 @security', async ({ api, config }, info) => {
    meta(info, 'ADMIN-003');
    const admin = await makeAdmin(api, config);
  ...
  test('TC-ADMIN-004-001 load-test seed/count/cleanup is idempotent with count bounds @admin @api @P2 @data-integrity', async ({ api, config }, info) => {
    meta(info, 'ADMIN-004');
    const admin = await makeAdmin(api, config);
    const traderU = makeUser('trader');
    await auth.registerAndLogin(api, traderU);
    try {
      expect((await a.loadTestSeed(api, admin.token, { trader_email: traderU.email, count: 501 })).status(), 'count le=500').toBe(422);
  ...
  test('TC-ADMIN-005-002 runtime config knobs persist to Redis with bounds + reset @admin @api @P1 @data-integrity', async ({ api, config }, info) => {
    meta(info, 'ADMIN-005');
    const admin = await makeAdmin(api, config);
    try {
  ...
  test('TC-ADMIN-005-001 admin test-SMS logs a send-attempt (log mode) and validates E.164 @admin @api @P1 @integration', async ({ api, config }, info) => {
  ...
  test('TC-ADMIN-001-005 DEF-ADMIN-001 — a shipped-label admin (lowercase user_role) 500s every admin route @admin @api @P0 @defect', async ({ api, config }, info) => {
    meta(info, 'ADMIN-001');
  ...
  test('TC-ADMIN-007-001 the orphaned /admin/dashboard route is not served by the backend (404) @admin @api @P2 @negative', async ({ api }, info) => {
    meta(info, 'ADMIN-007');
