...
test.describe('Fast-follow (no broker-behavior needed)', () => {
  ...
  test('TC-TRADE-001-002 limit BUY option with valid OCC fields is accepted @trading @api @P1 @positive', async ({
    api,
    config,
  }, info) => {
    meta(info, 'TRADE-001');
  ...
  test('TC-TRADE-001-004 inverted bracket geometry is rejected (422) @trading @api @P1 @boundary', async ({
    api,
    config,
  }, info) => {
    meta(info, 'TRADE-001');
  ...
  test('TC-COPY-001-004 excluded symbol is not mirrored @trading @api @P1 @data-integrity', async ({
    api,
    config,
  }, info) => {
    meta(info, 'COPY-001');
    ...
    try {
      ...
      await expect
        .poll(() => auditCount(config, 'copy.skipped_excluded_symbol'), { timeout: 20000 })
  ...
  test('TC-COPY-001-009 fanout stamps performance timestamps on the mirror @trading @api @P2 @observability', async ({
    api,
    config,
  }, info) => {
    meta(info, 'COPY-001');
