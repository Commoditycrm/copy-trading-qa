...
test.describe('Broker Integrations (offline)', () => {
  ...
  test('TC-BRK-001-001 list returns only the caller’s broker accounts @broker @api @P1 @security', async ({ api, config }, info) => {
    meta(info, 'BRK-001');
    ...
    try {
      ...
      expect((await theirs.json()).some((a: any) => a.id === p.brokerAccountId), 'no cross-user leak').toBe(false);
    } finally {
      p.cleanup();
    }
  });

  test('TC-BRK-001-009 broker=fake is not connectable via the public API (422 unknown broker) @broker @api @P1 @security', async ({ api, config }, info) => {
    meta(info, 'BRK-001');
  ...
  test('TC-BRK-001-010 connect with a missing credential block is rejected (422) @broker @api @P1 @negative', async ({ api, config }, info) => {
    meta(info, 'BRK-001');
    const p = await provisionFanout(api, config, []);
    try {
  ...
  test('TC-BRK-001-011 malformed credentials fail field-length validation (422) @broker @api @P1 @boundary', async ({ api, config }, info) => {
    meta(info, 'BRK-001');
    const p = await provisionFanout(api, config, []);
    try {
      const res = await brokers.connectRaw(api, p.traderAccess, {
        broker: 'alpaca', label: 'x', alpaca: { api_key: 'short', api_secret: 'short' }, // < min 8
      });
      expect(res.status()).toBe(422);
    } finally {
      p.cleanup();
  ...
  test('TC-BRK-001-012 broker credentials are Fernet-encrypted at rest @broker @api @P0 @security', async ({ api, config }, info) => {
    meta(info, 'BRK-001');
    const p = await provisionFanout(api, config, []);
    try {
  ...
  test('TC-BRK-001-013 broker credentials are never returned in responses @broker @api @P0 @security', async ({ api, config }, info) => {
    meta(info, 'BRK-001');
    const p = await provisionFanout(api, config, []);
    try {
      const res = await brokers.list(api, p.traderAccess);
      const acct = (await res.json()).find((a: any) => a.id === p.brokerAccountId);
      for (const leak of ['encrypted_credentials', 'credentials', 'api_key', 'api_secret', 'signing_key', 'access_token']) {
  ...
  test('TC-BRK-001-004 disconnect removes the account (204 + audit) @broker @api @P1 @integration', async ({ api, config }, info) => {
    meta(info, 'BRK-001');
    const p = await provisionFanout(api, config, []);
    try {
  ...
  test('TC-AUTHZ-001-017 another user cannot refresh/settings/disconnect your broker (404) @broker @api @P0 @security', async ({ api, config }, info) => {
    meta(info, 'AUTHZ-001', ['BRK-001']);
    const p = await provisionFanout(api, config, []);
    const attacker = await auth.registerAndLogin(api, makeUser('trader'));
    try {
      expect((await brokers.refreshBalance(api, attacker.access, p.brokerAccountId)).status()).toBe(404);
      expect((await brokers.updateSettings(api, attacker.access, p.brokerAccountId, { auto_pull_orders: false })).status()).toBe(404);
  ...
  test('TC-BRK-004-001 listener-gating settings persist + audit @broker @api @P1 @integration', async ({ api, config }, info) => {
    meta(info, 'BRK-004');
    const p = await provisionFanout(api, config, []);
    try {
      const res = await brokers.updateSettings(api, p.traderAccess, p.brokerAccountId, { auto_pull_orders: false, bring_open_orders: false });
  ...
  test('TC-BRK-003-001 SnapTrade webhook with a valid userId acks 200 @broker @api @P1 @integration', async ({ api, config }, info) => {
    meta(info, 'BRK-003');
    const p = await provisionFanout(api, config, []);
    try {
  ...
  test('TC-BRK-003-002 SnapTrade webhook is UNAUTHENTICATED — a forged call is accepted (poll-amplification finding) @broker @api @P1 @security', async ({ api }, info) => {
    meta(info, 'BRK-003');
    ...
    const res = await brokers.snaptradeWebhook(api, { userId: '00000000-0000-0000-0000-0000000000aa', eventType: 'ORDER_EXECUTED' });
    expect(res.status()).toBe(200);
    expect((await res.json()).ok).toBe(true);
  });

  test('TC-BRK-003-003 SnapTrade webhook with missing/non-UUID userId is acked with no action @broker @api @P2 @negative', async ({ api }, info) => {
    meta(info, 'BRK-003');
  ...
  test('TC-BRK-002-009 SnapTrade start + finish return 503 when the integration is not configured @broker @api @P2 @negative', async ({ api, config }, info) => {
    meta(info, 'BRK-002');
