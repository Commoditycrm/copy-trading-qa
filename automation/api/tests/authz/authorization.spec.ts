...
test.describe('AUTHZ-001 Role & ownership', () => {
  test('TC-AUTHZ-001-001 subscriber blocked from trader-only place-order (403) @auth @api @P1 @security', async ({ api }, info) => {
  ...
  test('TC-AUTHZ-001-002 trader blocked from subscriber-only settings (403) @auth @api @P1 @security', async ({ api }, info) => {
  ...
  test('TC-AUTHZ-001-003 non-admin blocked from admin endpoint (403) @auth @api @P1 @security', async ({ api }, info) => {
  ...
  test('TC-AUTHZ-001-004 accessing another user order returns 404 (ownership hidden) @auth @api @P1 @security', async ({ api }, info) => {
  ...
  test('TC-AUTHZ-001-005 forged JWT (wrong secret) → 401 @auth @api @P1 @security', async ({ api }, info) => {
    ...
    const forgedCfg = { envName: 'local' as const, apiBaseUrl: '', frontendUrl: '', brokerMode: 'fake' as const, jwtSecret: 'a-wrong-secret-not-the-server-one', paperAuthorized: false };
  ...
  test('TC-AUTHZ-001-007 missing token → 401 missing_token @auth @api @P1 @prod-safe @negative', async ({ api }, info) => {
    meta(info, 'AUTHZ-001');
  ...
  test('TC-AUTHZ-001-008 FE route-gap: subscriber direct API to trader action is blocked server-side @auth @api @P1 @security', async ({ api }, info) => {
    meta(info, 'AUTHZ-001', ['TRADE-001']);
    const { access } = await auth.registerAndLogin(api, makeUser('subscriber'));
