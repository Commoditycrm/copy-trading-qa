...
test.describe('SA-004 privilege escalation & tenant isolation', () => {
  ...
  test('SA-004 registration cannot mass-assign role=admin / is_active / email_verified @security @api @P0 @authz', async ({ api, config }, info) => {
    ...
    const res = await registerRaw(api, { ...u, is_active: false, email_verified: true, id: '00000000-0000-4000-8000-000000000001' } as Record<string, unknown>);
    try {
  ...
  test('SA-004 self-update (PATCH /api/auth/me) cannot change role or is_active @security @api @P1 @authz', async ({ api, config }, info) => {
    meta(info, 'AUTH-001');
    const u = makeUser('subscriber');
    const acct = await registerAndLogin(api, u);
    try {
      const res = await updateMe(api, acct.access, { role: 'trader', is_active: false, email_verified: true } as Record<string, unknown>);
  ...
  test('SA-004 a stranger cannot delete another tenant\'s broker account (IDOR) @security @api @P0 @authz', async ({ api, config }, info) => {
