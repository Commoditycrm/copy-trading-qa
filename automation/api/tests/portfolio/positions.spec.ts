...
test.describe('Positions', () => {
  ...
  test('TC-POS-001-001 list aggregates open positions from connected accounts @portfolio @api @P1 @integration', async ({
    api,
    config,
  }, info) => {
    meta(info, 'POS-001');
  ...
  test('TC-POS-001-003 close one position places the reverse order @portfolio @api @P1 @integration', async ({
    api,
    config,
  }, info) => {
    ...
    try {
      await mb.setPosition(p.brokerAccountId, [
        { symbol: 'AAPL', quantity: 10, avg_entry_price: 100, current_price: 110 },
      ]);
      try {
      ...
      await expect
        .poll(() => sideOrderCount(config, p.traderId, 'AAPL', 'sell'), { timeout: 20000 })
  ...
  test('TC-POS-001-004 close guards — ownership 404, missing position 404, quantity bounds 422 @portfolio @api @P1 @security', async ({
    api,
    config,
  }, info) => {
    meta(info, 'POS-001');
    ...
    try {
      ...
      // quantity bounds
      expect((await pos.closePosition(api, p.traderAccess, 'AAPL', p.brokerAccountId, { quantity: 0 })).status()).toBe(
        422,
      );
      expect((await pos.closePosition(api, p.traderAccess, 'AAPL', p.brokerAccountId, { quantity: 99 })).status()).toBe(
        422,
      );
    } finally {
      p.cleanup();
  ...
  test('TC-POS-001-007 trader bulk-close subscribers is queued async @portfolio @api @P1 @integration', async ({
    api,
    config,
  }, info) => {
