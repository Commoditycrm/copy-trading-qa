...
function resetTokenFor(userId) {
  ...
  const re = /token=([A-Za-z0-9._-]{20,})/g; let m, found = null;
  while ((m = re.exec(logs))) { try { if (jwt.decode(m[1])?.sub === userId) found = m[1]; } catch {} }
  return found;
}
...
out.controls.register_weak_pw = (await post('/api/auth/register',
  { email: `qa+ctl-${Date.now()}@qa.kopyya.dev`, password: 'abcdefgh', role: 'subscriber' }, { 'X-Forwarded-For': ip() })).status;
...
for (let i = 0; i < 2; i += 1) {
  const email = `qa+defB-${Date.now()}-${i}@qa.kopyya.dev`; const xff = { 'X-Forwarded-For': ip() };
  ...
  let tok = null; for (let t = 0; t < 8 && !tok; t += 1) { await sleep(1000); tok = resetTokenFor(id); }
...
for (let i = 0; i < 2; i += 1) {
  const email = `qa+defC-${Date.now()}-${i}@qa.kopyya.dev`; const xff = { 'X-Forwarded-For': ip() };
  const reg = await post('/api/auth/register', { email, password: 'Qa!Strong123', role: 'subscriber' }, xff);
  const id = JSON.parse(reg.body).id;
  ...
  let lowerTok = null; for (let t = 0; t < 6 && !lowerTok; t += 1) { await sleep(1000); lowerTok = resetTokenFor(id); }
