...
export class MockBroker {
  constructor(
    ...
    private readonly runId: string = RUN_ID,
  ) {
    assertLocal(cfg);
  }
  ...
  setOrderStatus(
    ...
    return admin('POST', '/admin/order-status', {
      run_id: this.runId,
      order_id: orderId,
      status,
      filled_quantity,
  ...
  private drive(action: string, args: object, container: 'backend' | 'worker' = 'backend'): any {
    ...
    execSync(`docker compose -f "${compose}" cp "${driver}" ${container}:/tmp/qa_driver.py`, {
  ...
  configureBracketScenario(
    entryOrderId: string,
    fill: { quantity: number | string; price: number | string },
  ): Promise<any> {
  ...
  emitEntryFill(
    accountId: string,
    entryOrderId: string,
  ): { legs: Array<{ id: string; bracket_leg: string; status: string }> } {
  ...
  emitPartialFill(accountId: string, orderId: string, quantity: number | string, price: number | string): any {
    return (async () => {
      await this.setOrderStatus(orderId, 'partially_filled', quantity, price);
      return this.syncFills(accountId);
    })();
  }
  emitFullFill(accountId: string, orderId: string, quantity: number | string, price: number | string): any {
    return (async () => {
      await this.setOrderStatus(orderId, 'filled', quantity, price);
      return this.syncFills(accountId);
    })();
  }
  /** Fill a TP leg and run OCO sibling-cancel → returns sibling state. */
  emitTakeProfitFill(tpLegId: string): {
    sibling_cancelled: boolean;
    legs: Array<{ id: string; bracket_leg: string; status: string }>;
  } {
    return this.drive('fill_leg_oco', { leg_order_id: tpLegId });
  }
  emitStopLossFill(slLegId: string): {
    sibling_cancelled: boolean;
    legs: Array<{ id: string; bracket_leg: string; status: string }>;
  } {
  ...
  setPnlSnapshot(
    accountId: string,
    snapshot: { todays_pl?: number; equity?: number; beginning_day_balance?: number; todays_realized?: number },
  ): Promise<any> {
  ...
  pollerPass(accountIds: string[]): {
    accounts: Record<string, { copy_enabled: boolean | null; auto_liquidated: boolean | null; paused: boolean | null }>;
  } {
  ...
  seedPendingChild(
    ...
    opts: { symbol?: string; quantity?: number } = {},
  ): { child_id: string } {
    return this.drive('seed_pending_child', {
      user_id: userId,
  ...
  retryHeartbeat(): {
    heartbeat: { running: boolean; healthy: boolean; last_run_at: string | null; seconds_since: number | null };
  } {
